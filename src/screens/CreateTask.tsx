  ...
  const getNames = () => {
    ...
    if (Array.isArray(inputs?.assigned_to)) {
      inputs?.assigned_to.map(id => {
        concat +=
          projectMembers.find(pm => id === pm.id).first_name +
          ' ' +
          projectMembers.find(pm => id === pm.id).last_name +
          '    ';
      });
    }
    return concat;
    ...
    <>
      ...
      <ActionSheet ref={actionMemberSheetRef}>
        <View style={{marginBottom: 30}}>
          ...
          <View>
            {projectMembers.map(({id, first_name, last_name}) => (
              <TouchableWithoutFeedback
                onPress={async () => {
                  if (Array.isArray(inputs?.assigned_to)) {
                    if (inputs?.assigned_to.includes(id)) {
                      update(
                        [...inputs?.assigned_to].filter(ass => ass !== id),
                        'assigned_to',
                      );
                      return;
                    }
                ...
                <View style={{padding: 10, flexDirection: 'row'}}>
                  <>
                    {Array.isArray(inputs?.assigned_to) &&
                    inputs?.assigned_to.includes(id) ? (
                      <SimpleLineIcons name="check" size={24} />
                    ) : (
